...
class JackTokenizer:
	...
	def advance(self):
		...
		self._currentToken = self.__nextTokens.pop(0)	# set the new current token from the list of next tokens
		if self.__nextTokens != []:
			self.__nextToken = self.__nextTokens[0]
		else:
			self.__nextToken = None
		## print(self.__currentToken)
		self._resetProperties()						# sets the current properties to None
		self._tokenType = self.currentToken		# sets the  properties for the current token
		if self.__emitXML:
			self._writeXML()
	...
	def _writeXML(self):
		...
		if self.tokenType == C_KEYWORD:
			self._tokenXML = "<keyword> " + self._keyword + " </keyword>"
			#self.__xmlfile.writelines("<keyword> " + self._keyword + " </keyword>\n")
			
		elif self.tokenType == C_SYMBOL:
			self._tokenXML = "<symbol> " + self._escapeSymbol(self._symbol) + " </symbol>"

			#self.__xmlfile.writelines("<symbol> " + self._escapeSymbol(self._symbol) + " </symbol>\n")
				
		elif self.tokenType == C_LITERAL_STRING:
			self._tokenXML = "<stringConstant> " + self._stringVal + " </stringConstant>"
			#self.__xmlfile.writelines("<stringConstant> " + self._stringVal + " </stringConstant>\n")
			
		elif self.tokenType == C_INTEGER:
			self._tokenXML = "<integerConstant> " + self._intVal + " </integerConstant>"
			#self.__xmlfile.writelines("<integerConstant> " + self._intVal + " </integerConstant>\n")
			
		elif self.tokenType == C_IDENTIFIER:
			self._tokenXML = "<identifier> " + self._identifier + " </identifier>"
			#self.__xmlfile.writelines("<identifier> " + self._identifier + " </identifier>\n")
				
		self.__xmlfile.writelines(self.tokenXML + "\n")
		return
	...
	def hasMoreTokens(self):
		...
		if len(self.__nextTokens) > 0:			# there are more tokens on the current line of Jack code to process
			#print("nextToken",self.__nextToken)
			return(True)
	...
	@property
	def tokenXML(self):
		return self.__tokenXML

	@tokenXML.setter
	def _tokenXML(self, s):
		# print("Setting token XML to:", s)
		self.__tokenXML = s
	...
	@currentToken.setter
	def _currentToken(self, s):
		# print("Setting current token to:", s)
		self.__currentToken = s
	...
	@nextToken.setter
	def _nextToken(self, s):
		# print("Setting next token to:", s)
		self.__nextToken = s
